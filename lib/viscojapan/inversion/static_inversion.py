...
class StaticInversion(Inversion):
    ...
    def set_data_sd(self):
        sig_ep = EpochDisplacementSD(self.file_sd, self.filter_sites_file)
        self.sd = sig_ep[self.epoch]
        assert_col_vec_and_get_nrow(self.sd)

    def set_data_G(self):
        G_ep = EpochG(self.file_G, self.filter_sites_file)
        self.G = G_ep[0]

    def set_data_d(self):
        d_ep = EpochDisplacement(self.file_d, self.filter_sites_file)
